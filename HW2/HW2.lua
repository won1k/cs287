...
cmd:option('-pretrained', false, 'pretrained features')
...
function NNPre(train_input_word_windows, train_input_cap_windows, train_output, alpha, dword, dcaps, lambda, epochs, bsize, dhid)
	local n, dwin = train_input_word_windows:size()[1], train_input_word_windows:size()[2]
...
function main()
    ...
    local valid_input_word_windows = f:read('valid_input_word_windows'):all():long()
    local valid_input_cap_windows = f:read('valid_input_cap_windows'):all():long()
    ...
    elseif clf == 'nn' then
    	...
    	NNPre(train_input_word_windows, train_input_cap_windows, train_output, alpha, dword, dcaps, lambda, epochs, bsize, dhid)
    end
